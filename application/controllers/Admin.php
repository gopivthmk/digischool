...
class Admin extends CI_Controller
{
    ...
    function teacher($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['password']    = sha1($this->input->post('password'));
            $data['nationality']  			= $this->input->post('nationality');
            $data['caste_community']  			= $this->input->post('caste_community');
            $data['martial_status']  			= $this->input->post('martial_status');
            $data['spouse_name']  			= $this->input->post('spouse_name');
            $data['occupation']  			= $this->input->post('occupation');
            $data['spouse_mobile_no']  			= $this->input->post('spouse_mobile_no');
            $data['blood_group']  			= $this->input->post('blood_group');
            $data['personal_identification_mark']  			= $this->input->post('personal_identification_mark');
            $data['medications']  			= $this->input->post('medications');
            $data['edu_qualifications']  			= $this->input->post('edu_qualifications');
            $data['tech_qualifications']  			= $this->input->post('tech_qualifications');
            $data['previous_exp']  			= $this->input->post('previous_exp');
            $data['ration_card_no']  			= $this->input->post('ration_card_no');
            $data['aadhar_card_no']  			= $this->input->post('aadhar_card_no');
            $data['voter_id_no']  			= $this->input->post('voter_id_no');
            $data['pan_bank_account_details']  			= $this->input->post('pan_bank_account_details');
            $data['pf_no']  			= $this->input->post('pf_no');
            $data['esi_no']  			= $this->input->post('esi_no');
            $data['total_family_members']  			= $this->input->post('total_family_members');
            $data['total_no_of_childrens']  			= $this->input->post('total_no_of_childrens');
            $data['details_of_childrens']  			= $this->input->post('details_of_childrens');
    ...
    function create_fees_category()
    {
        ...
        $data['page_name']  = 'create_fees_category';
        $data['page_title'] = get_phrase('create_fees_category');
        $this->load->view('backend/index', $data);
    }

    function edit_fees_category($param1)
    {
    //  echo "TEhis".$param1;exit;
        if ($this->session->userdata('admin_login') != 1)
        {
            $this->session->set_userdata('last_page', current_url());
            redirect(base_url(), 'refresh');
       ...
       $data['edit_data'] = $this->db->get_where('fees_master_category', array(
        ...
        $data['page_title'] = get_phrase('edit_fees_category');
        $this->load->view('backend/index', $data);
    }
